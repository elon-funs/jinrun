	...
	<div class="col-xs-12 search-row">
		<div class="panel-body">
            ...
            <li <?php if($state == -1){ ?>class="active" <?php } ?>>
				<a href="<?php echo U('SellerManage/apply' ,array('state' => -1));?>">全部</a>
			</li>
            <li <?php if($state == 0){ ?>class="active" <?php } ?>>
				<a href="<?php echo U('SellerManage/apply', array('state' => 0));?>">待审核</a>
			</li>              
            <li <?php if($state == 1){ ?>class="active" <?php } ?>>
				<a href="<?php echo U('SellerManage/apply', array('state' => 1));?>" >已审核</a>
			</li> 
			<li <?php if($state == 2){ ?>class="active" <?php } ?>>
				<a href="<?php echo U('SellerManage/apply', array('state' => 2));?>" >已拒绝</a>
			</li>  			
			...
			<table class="table table-striped table-bordered table-hover">
				...
				<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><tr>							
							...
							<td>
								<?php  if($m['type'] == 0) { echo '<span class="green bold">个人</span>'; }else if($m['type'] == 1) { echo '<span class="red bold">企业</span>'; } ?>
							</td>
							<td><?php echo ($m["category_name"]); ?></td>
							<td>
							<?php
 switch($m['state']){ case '1': echo '<span class="green bold">通过</span>'; break; case '2': echo '<span class="blue bold">拒绝</span>'; break; case '0': echo '<span class="red bold">申请中</span>'; break; } ?>
							</td>
							<td>
								<?php echo date('Y-m-d H:i:s', $m['addtime']); ?>
							</td>
							<td>
								<a  class="btn btn-xs btn-info" href='<?php echo U("SellerManage/apply_detail",array("id"=>$m["id"]));?>'>
									<i class="icon-eye-open bigger-120"></i>查看资料
								</a> 
								
								<?php if($m['state'] == 0){ ?>
								<a class="delete btn btn-xs btn-danger" href='<?php echo U("SellerManage/shenhe",array("id"=>$m["id"],"state" => 1));?>' >
									审核通过
								</a>
								
								<a class="delete btn btn-xs btn-danger" href='<?php echo U("SellerManage/shenhe",array("id"=>$m["id"],"state" => 2));?>' >
									残忍拒绝
								</a>
								<?php } ?>
							</td>
