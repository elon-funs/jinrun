...
<div class="row">
	...
	<thead>
		<th><input name="name" type="text" placeholder="输入商家名称" value="<?php echo I('name');?>" />
			&nbsp;&nbsp;
			<a class="btn btn-primary" href="javascript:;" id="search" url="<?php echo U('Balance/assets');?>">查询</a>
		</th>
			...
			<table class="table table-striped table-bordered table-hover">
				...
				<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>						
							...
							<td>
								<?php if($data['state'] == 0){ ?>
								<a class="delete btn btn-xs btn-danger" href='<?php echo U("Balance/suretixianmoney",array("id"=>$data["id"]));?>' >
...
<script>
var shenurl = "<?php echo U('Balance/shenqing');?>";
$(function(){
